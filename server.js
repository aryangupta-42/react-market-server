...
// CONNECT TO MONGODB
const MONGODB_URI =
    process.env.MONGODB_URI_LOCAL || 'mongodb://localhost:27017/market';
    ...
    try {
        ...
        if (searchRes !== null) {
            if (!searchRes.username) {
                res.status(401).json(
                    response(false, 'Whoops!! No such user exists.', null)
                );
            } else {
                ...
                if (!access) {
                    ...
                    res.status(200).json(
                        response(true, 'Login successful', jsonData)
                    );
                }
            }
        } else {
...
app.get('/initial/sudo/createSuperAdmin', async (_, res) => {
    ...
    try {
        await admin.save();
        res.status(201).json(
            response(true, 'Admin created Successfully', admin)
        );
    } catch (err) {
        console.log(err);
